...
def new_beer(request):
    if request.method == "POST":
        ...
        if beer.is_valid():
            ...
            if BeerModel.objects.filter(user=new_beer.user,
                                        created__gt=yesterday).exists():
                beer = NewBeerForm()
                return render(request,
                              'beer_tracker/new_beer.html',
                              {'invalid': True, 'beer': beer})
            new_beer.save()
            return HttpResponseRedirect(reverse('home'))
    else:
        beer = NewBeerForm()
...
def user_login(request):
    if request.method == "POST":
        ...
        if user:
            ...
            user_form = UserForm()
            return render(request,
                          'beer_tracker/index.html',
                          {'invalid': True, 'user_form': user_form})
